...
var img1=preload("res://抽奖系统/sprite/NES - Snow Brothers Snow Bros Nick & Tom - Machine Bonus_33.png")
var img2=preload("res://抽奖系统/sprite/NES - Snow Brothers Snow Bros Nick & Tom - Machine Bonus_34.png")
var img3=preload("res://抽奖系统/sprite/NES - Snow Brothers Snow Bros Nick & Tom - Machine Bonus_29.png")
var img4=preload("res://抽奖系统/sprite/NES - Snow Brothers Snow Bros Nick & Tom - Machine Bonus_30.png")
var img5=preload("res://抽奖系统/sprite/NES - Snow Brothers Snow Bros Nick & Tom - Machine Bonus_32.png")
var img6=preload("res://抽奖系统/sprite/NES - Snow Brothers Snow Bros Nick & Tom - Machine Bonus_31.png")

var currState


@onready var vbox=$vbox
var card=preload("res://抽奖系统/scene/card.tscn")
var slotList=[]
...
func _physics_process(delta):
	if currState==Game.state.scroll:
		...
		if vbox.get_child_count()>0:
			...
			if node.position.y<-boxSize:	#如果超过了屏幕就往地下增加一个
				...
				temp.position.y=last.position.y+boxSize
				vbox.add_child(temp)
		...
		if speed>0:
			...
			if vbox.get_child_count()>0:
				...
				var yPos=0
				var idx=0
				if node.position.y<-boxSize/2:  #如果第一个位置超过屏幕外就继续让它滚动到 屏幕外
					yPos=-boxSize
				for i in vbox.get_children():
					var temp=create_tween()
					temp.tween_property(i,'position:y',yPos+boxSize*idx,1)
					if idx==0:
						temp.tween_callback(getResult)
					idx+=1
...
func getResult():
	if vbox.get_child_count()>0:
		var node=vbox.get_child(0)
		if node.position.y<0:  #把这个超过屏幕的删除了
			vbox.remove_child(node)
